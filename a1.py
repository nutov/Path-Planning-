...
class BreadthFirstSearch:
    ...
    def get_possible_moves(self):
        ...
        for move in potential_moves:
            ...
            if (str(move) not in self.explored.keys()) and (
                str(move) not in self.not_explored.keys()) :
    ...
    def goal_found(self):
        if self.goal_str in self.not_explored.keys():
            ...
            self.path[self.pos[0],self.pos[1]] = self.not_explored[self.goal_str]
    ...
    def explore_next_move(self):
        ...
        # sort the values in the not_explored dictionary by their depth
        # so that the next move we will develop will have the minimal depth
        # returns the keys by the sorted order
        sorted_not_explored = sorted(
            self.not_explored,
            key=self.not_explored.get,
            reverse=False)

        # filter the common dict values
        
        sorted_not_explored = list(filter(lambda item : item not in list(self.explored.keys()),sorted_not_explored))
        ...
        if len(sorted_not_explored) > 0:
            ...
            self.pos_depth = self.not_explored[sorted_not_explored[0]]
