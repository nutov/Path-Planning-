...
class AStarSearch:
    ...
    def get_possible_moves(self):
        # Get Potential Moves
        possible_move = self.generate_potential_moves(self.pos)
        ...
        for move in possible_move:
            ...
            if (str(move) not in self.explored.keys()) and (str(move) not in self.not_explored.keys()):
                self.not_explored[str(move)] = self.pos_depth + 1 + self.heuristic(move,ord = 1)
    ...
    def goal_found(self):
        if self.goal_str in self.not_explored.keys():
            # Add goal to path.
            goal = self.string_to_array(self.goal_str)
            self.path[goal[0],goal[1]] = self.not_explored.pop(self.goal_str)
    ...
    def explore_next_move(self):
        ...
        sort_moves = list(dict(sorted(self.not_explored.items(), key=lambda x: x[1], reverse=False)).keys())
        
        # Determine the pos and depth of next move.
        self.pos_str = sort_moves[0]
        
        self.pos = self.string_to_array(self.pos_str)

        self.pos_depth = self.not_explored.pop(sort_moves[0]) - self.heuristic(self.pos,ord =1)
        
        # Write depth of next move onto path.
        self.path[self.pos[0], self.pos[1]] = round(self.pos_depth, 1)
        return True

    def heuristic(self, move:np.array,ord = None):
        goal = self.string_to_array(self.goal_str)
        answer = np.linalg.norm(goal - move, ord= ord)
